...
class KempToolkit(BaseToolkit):
    def __init__(self,bert_model,model_path,vocabulary_path,device):
        super(KempToolkit, self).__init__(bert_model,model_path,vocabulary_path,device)
        # self.plm = PlmAutoModel(bert_model)
        self.args = load_params()
        with open(vocabulary_path,"rb") as f:
            vocab = json.load(f)
        self.model = KempModel(args=self.args, vocab=vocab,decoder_number=32)
    ...
    def run(self,sentence):
        ...
        item = Dataset(datable, self.vocabulary.label_set, self.args)
...
if __name__ == '__main__':
    toolkit = KempToolkit(bert_model=None,model_path='/data/zhaojingxuan/zjxcode/CogAgent/datapath/kemp/experimental_result/simple_test--2023-01-30--17-28-17.80/model/checkpoint-50000/models.pt',
                            vocabulary_path='/data/zhaojingxuan/zjxcode/CogAgent/datapath/kemp/raw_data/vocab.json',
                            device=torch.device("cuda:6"),
                            )
    sentence = "i remember going to see the fireworks with my best friend . it was the first time we ever spent time alone together . although there was a lot of people , we felt like the only people in the world ."
