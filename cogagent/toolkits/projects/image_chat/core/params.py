...
from core.build_data import modelzoo_path
from core.loader import load_teacher_module, load_agent_module, load_world_module
...
def print_announcements(opt):
    ...
    if ic.PathManager.exists(noannounce_file):
...
def get_model_name(opt):
    ...
    if model is None:
        ...
        if model_file is not None:
            ...
            if ic.PathManager.exists(optfile):
                new_opt = ic.Opt.load(optfile)
...
class ParlaiParser(argparse.ArgumentParser):
    ...
    def add_model_subargs(self, model: str, partial: ic.Opt):
        """
    ...
    def add_task_args(self, task: str, partial: ic.Opt):
        """
        Add arguments specific to the specified task.
        """
        for t in ic.ids_to_tasks(task).split(','):
    ...
    def _load_known_opts(self, optfile, parsed):
        ...
        """
        new_opt = ic.Opt.load_init(optfile)
    ...
    def _process_args_to_opts(self, args_that_override: Optional[List[str]] = None):
        self.opt = ic.Opt(vars(self.args))
        extra_ag = []
