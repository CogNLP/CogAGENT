...
from tasks.personality_captions.build import build
from core.agents import Agent
from core.dict import DictionaryAgent
...
class TransresnetAgent(Agent):
    ...
    def add_cmdline_args(
        cls, parser: ic.ParlaiParser, partial_opt: Optional[ic.Opt] = None
    ):
        """
    ...
    def _build_model(self, path=None):
        ...
        if self.opt.get('init_model') and ic.PathManager.exists(self.opt['init_model']):
            init_model_path = self.opt['init_model']
        elif self.opt.get('model_file') and ic.PathManager.exists(self.opt['model_file']):
    ...
    def _setup_cands(self):
        ...
        if self.fcp is not None:
            with ic.PathManager.open(self.fcp) as f:
                self.fixed_cands = [c.replace('\n', '') for c in f.readlines()]
            cands_enc_file = '{}.cands_enc'.format(self.fcp)
            print('loading saved cand encodings')
            if ic.PathManager.exists(cands_enc_file):
                with ic.PathManager.open(cands_enc_file, 'rb') as f:
                ...
                ic.atomic_save(self.fixed_cands_enc, cands_enc_file)
    ...
    def load_personalities(self):
        ...
        perss = []
        with ic.PathManager.open(personality_path) as f:
            for line in f:
    ...
    def report(self):
        ...
        if self.metrics['num_samples'] > 0:
            m['hits@1/100'] = ic.round_sigfigs(
                self.metrics['hits@1/100'] / self.metrics['num_samples'], 4
            )
            m['loss'] = ic.round_sigfigs(
    ...
    def save(self, path=None):
        ...
        ic.atomic_save(states, path)

        with ic.PathManager.open(path + '.opt', 'w') as handle:
    ...
    def load(self, path):
        ...
        """
        with ic.PathManager.open(path, 'rb') as f:
