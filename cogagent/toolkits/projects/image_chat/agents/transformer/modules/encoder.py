...
from projects.image_chat import image_chat as ic
from agents.transformer.modules import (
...
from agents.transformer.modules.modular import swappable
# from parlai.core.opt import Opt
# from parlai.core.params import default
# from parlai.utils.misc import warn_once
# from parlai.utils.torch import PipelineHelper
from utils.fsdp import fsdp_wrap
from nn.checkpoint import checkpoint_wrapper
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        n_heads = ic.default(n_heads, opt['n_heads'])
        embedding_size = ic.default(embedding_size, opt['embedding_size'])
        ffn_size = ic.default(ffn_size, opt['ffn_size'])
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.embeddings_scale = ic.default(
            embeddings_scale, opt.get('embeddings_scale', False)
        )
        self.reduction_type = reduction_type
        self.padding_idx = padding_idx
        # this is --dropout, not --relu-dropout or --attention-dropout
        self.dropout_frac = ic.default(dropout, opt.get('dropout', 0.0))
        self.dropout = nn.Dropout(p=self.dropout_frac)
        self.activation = ic.default(activation, opt.get('activation', 'relu'))
        self.variant = ic.default(variant, opt.get('variant', 'aiayn'))
        self.n_segments = ic.default(n_segments, opt.get('n_segments', 0))

        self.n_positions = ic.default(n_positions, get_n_positions_from_options(opt))
        ...
        self.output_scaling = ic.default(output_scaling, opt.get('output_scaling', 1.0))
    ...
    def _apply_model_parallel(self, tensor, mask):
        ...
        """
        chunks = ic.PipelineHelper.split((tensor, mask))
        work_items = ic.PipelineHelper.schedule_work_items(self.layers, chunks)

        for chunk_idx, layer_nos, next_device in work_items:
            s_tensor, s_mask = chunks[chunk_idx]
            for layer_no in layer_nos:
                s_tensor = self.layers[layer_no](s_tensor, s_mask)
            chunks[chunk_idx] = ic.PipelineHelper.chunk_to((s_tensor, s_mask), next_device)

        tensor_out, mask_out = ic.PipelineHelper.join(chunks)
        return tensor_out
