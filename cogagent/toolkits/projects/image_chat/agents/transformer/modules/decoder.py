...
from core.torch_agent import DictionaryAgent
...
class BaseTransformerDecoder(nn.Module, ABC):
    ...
    def _apply_model_parallel(
        ...
        """
        chunks = ic.PipelineHelper.split((tensor, *extra_args, incr_state))
        ...
        for chunk_idx, layer_nos, next_device in work_items:
            ...
            s_layer_args = ic.PipelineHelper.chunk_to(
                (s_tensor, *s_extra_args), next_device
            )
            chunks[chunk_idx] = (*s_layer_args, s_incr_state)

        tensor_out = ic.PipelineHelper.join([c[0] for c in chunks])
        new_incr_state = {
            layer_no: ic.PipelineHelper.join(pieces)
