...
class BasicAttention(nn.Module):
    ...
    def forward(
        ...
        if mask_ys is not None:
            ...
            l1.masked_fill_(attn_mask, ic.neginf(l1.dtype))
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        self, opt: ic.Opt, n_heads: int = None, dim: int = None, dropout: float = 0
    ):
        super(MultiHeadAttention, self).__init__()

        n_heads = ic.default(n_heads, opt['n_heads'])
        dim = ic.default(dim, opt['embedding_size'])
    ...
    def forward(  # type: ignore
        ...
        **kwargs,
        ...
        dot_prod.masked_fill_(attn_mask, ic.neginf(dot_prod.dtype))
