...
class GenerationMixin:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
            self, input_ids: torch.LongTensor, token_type_ids: torch.LongTensor, per_input_ids: torch.LongTensor, model_kwargs
    ) -> Dict[str, Any]:
        ...
        model_kwargs["encoder_outputs"]: ModelOutput = encoder(input_ids, token_type_ids=token_type_ids, output_hidden_states=True, return_dict=True, **encoder_kwargs)
        model_kwargs["encoder_input_ids"] = input_ids
        model_kwargs["per_input_ids"] = per_input_ids
        return model_kwargs
    ...
    def generate(
            ...
            use_decoder2: bool = False,
            per_input_ids: Optional[torch.LongTensor] = None,
            **model_kwargs
    ) -> torch.LongTensor:
        r"""
        ...
        use_decoder2 = use_decoder2 if use_decoder2 is not None else False
        ...
        if self.config.is_encoder_decoder:
            ...
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(input_ids, token_type_ids, per_input_ids, model_kwargs)
    ...
    def greedy_search(
        ...
        input_ids_2 = input_ids.clone().detach()
        ...
        while cur_len < max_length:
            ...
            model_inputs = self.prepare_inputs_for_generation(input_ids, token_type_ids, **model_kwargs)

            assert model_inputs['input_ids'] is not None
            assert model_inputs['per_input_ids'] is not None

            # forward pass to get next token
            outputs, _, _ = self(**model_inputs, output_hidden_states=True, return_dict=True)
        ...
        if use_decoder2:
            ...
            # forward pass to get full representation
            assert input_ids is not None
            model_inputs = self.prepare_inputs_for_generation(input_ids, token_type_ids, **model_kwargs)
            assert model_inputs['per_input_ids'] is not None

            outputs, _, _ = self(**model_inputs, output_hidden_states=True, return_dict=True)
            decoder_generated_outputs = outputs.decoder_hidden_states[-1]
            decoder2_inputs_embeds = outputs.decoder_hidden_states[0]

            sequence_lengths, unfinished_sequences, cur_len = self._init_sequence_length_for_generation(
                input_ids_2, max_length
            )

            while cur_len < max_length:

                # prepare model inputs
                model_inputs = self.prepare_inputs_for_generation(input_ids_2, token_type_ids, **model_kwargs)
                model_inputs["decoder2_inputs_embeds"] = decoder2_inputs_embeds
                model_inputs["decoder_generated_outputs"] = decoder_generated_outputs
                model_inputs["stage2"] = True

                # forward pass to get next token
                _, outputs, _ = self(**model_inputs, return_dict=True)

                next_token_logits = outputs.logits[:, -1, :]

                # pre-process distribution
                scores = logits_processor(input_ids_2, next_token_logits)
                ...
                # add token and increase length by one
                input_ids_2 = torch.cat([input_ids_2, next_tokens[:, None]], dim=-1)
        ...
        return input_ids_2 if use_decoder2 else input_ids
    ...
    def sample(
        ...
        while cur_len < max_length:
            ...
            # forward pass to get next token
            outputs_1, outputs_2 = self(**model_inputs, return_dict=True)

            outputs = outputs_2 if use_decoder2 else outputs_1
