...
from cogagent.models.default_policy import DefaultPolicy
...
class PolicyModel(BaseModel):
    def __init__(self, args, input_lang_index2word, output_lang_index2word, input_lang_word2index, output_lang_word2index,dev_data):
    ...
    def forward(self, input_tensor, input_lengths, target_tensor, db_tensor, bs_tensor):
        ...
        # ENCODER
        encoder_hidden = self.encoder(input_tensor, input_lengths)
    ...
    def decode(self, target_tensor, decoder_hidden):
        ...
        if self.beam_search:  # wenqiang style - sequicity
            ...
            decoded_sentences = self.greedy_decode(decoder_hidden, target_tensor)
            return decoded_sentences

    def greedy_decode(self, decoder_hidden, target_tensor):
        decoded_sentences = []
        batch_size, seq_len = target_tensor.size()
        decoder_input = torch.LongTensor([[SOS_token] for _ in range(batch_size)], device=self.device)
        # decoder_input = torch.LongTensor([[0] for _ in range(batch_size)], device=self.device)

        decoded_words = torch.zeros((batch_size, self.max_len))
        for t in range(self.max_len):
            decoder_output, decoder_hidden = self.decoder(decoder_input, decoder_hidden)

            # topv, topi = decoder_output.data.topk(1)  # get candidates到这里topi=[[4],[4],[4],[4],..,[4]]
            # 源码topi=[[9],[9],[9],[9],[54],[8],[9],[9],[9],[5]]
            topv, topi = decoder_output.topk(1)
            topi = topi.view(-1)
    ...
    # 每个json全都以[32,60]形状存到batch中，batch_size=1时只有一个json，取出来对应的句子数量和长度
    def loss(self, batch, loss_function):
        input_lengths =[ batch["input_lengths"][0][i] for i in range(32) if batch["input_lengths"][0][i] != 1 ]
        longest_sent = max(input_lengths)
        batch_size = len(input_lengths)
        padded_tensor = np.ones((batch_size, longest_sent)) * PAD_token
        for i, x_len in enumerate(input_lengths):
            sequence = batch["input_tensor"][0][i]
            padded_tensor[i, 0:x_len] = sequence[0:x_len]
        input_tensor = torch.LongTensor(padded_tensor)
        # input_tensor = batch["input_tensor"][0]
        # target_tensor = batch["target_tensor"][0]
        target_lengths =[ batch["target_lengths"][0][i] for i in range(32) if batch["target_lengths"][0][i] != 1 ]
        longest_sent = max(target_lengths)
        batch_size = len(target_lengths)
        padded_tensor = np.ones((batch_size, longest_sent)) * PAD_token
        for i, x_len in enumerate(target_lengths):
            sequence = batch["target_tensor"][0][i]
            padded_tensor[i, 0:x_len] = sequence[0:x_len]
        target_tensor= torch.LongTensor(padded_tensor)
        db_tensor = batch["db_tensor"][0][0:len(input_lengths)]
        bs_tensor = batch["bs_tensor"][0][0:len(input_lengths)]
        # input_lengths = batch["input_lengths"][0]
        proba, _, decoded_sent = self.forward(input_tensor, input_lengths, target_tensor, db_tensor, bs_tensor)
        proba = proba.view(-1, self.vocab_size)
        loss = loss_function(proba, target_tensor.view(-1))
        return loss

    def evaluate(self, batch, metric_function):
        val_dials = self.dev_data.datas['val'][0]
        val_dials_gen = {}
        num = 0
        for name,val_file in val_dials.items():
            ...
            with torch.no_grad():
                ...
                decoder_hidden = self.policy(encoder_hidden, db_tensor, bs_tensor)
                    ...
                    # GENERATION
                decoded_words = self.decode(target_tensor, decoder_hidden)          
                val_dials_gen[name] = decoded_words
            num = num + 1

        metric_function.evaluate(val_dials_gen , val_dials)
