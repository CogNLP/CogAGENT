...
import numpy as np
import math
from cogagent.models.kemp_common_layer import EncoderLayer, DecoderLayer, MultiHeadAttention, Conv, PositionwiseFeedForward, LayerNorm , _gen_bias_mask ,_gen_timing_signal, share_embedding, LabelSmoothing, NoamOpt, _get_attn_subsequent_mask
import random
...
class KempModel(BaseModel):
    ...
    def loss(self, batch, loss_function):
        ...
        loss_emotion = loss_function["crossentropyloss"](emotion_logit, batch['emotion_label'])
        ...
        loss = loss_function["nllloss"](logit.contiguous().view(-1, logit.size(-1)),
        ...
        # loss_ppl = loss_function["nllloss"](logit.contiguous().view(-1, logit.size(-1)),
        #                                   dec_batch.contiguous().view(-1) if self.args.pointer_gen else dec_ext_batch.contiguous().view(-1)).item()


        # if self.args.label_smoothing:
        #     # return loss_ppl, math.exp(min(loss_ppl, 100)), loss_emotion.item(), 
        # return loss_ppl
        # else:
    ...
    def evaluate(self, batch, metric_function):
        ...
        gdn = {}
        itr = 0
        l = len(batch['context_batch'])
        for j in range(l):
            one_batch['context_batch'] = batch['context_batch'][j][None, :]
            one_batch['context_ext_batch'] = batch['context_ext_batch'][j][None, :]
            one_batch['context_lengths'] = batch['context_lengths'][j]
            one_batch['mask_context'] = batch['mask_context'][j][None, :]
            one_batch['context_vads'] = batch['context_vads'][j][None, :]
            one_batch['context_vad'] = batch['context_vad'][j][None, :]
            one_batch['concept_batch'] = batch['concept_batch'][j][None, :]
            one_batch['concept_ext_batch'] = batch['concept_ext_batch'][j][None, :]
            one_batch['concept_lengths'] = batch['concept_lengths'][j]
            one_batch['mask_concept'] = batch['mask_concept'][j][None, :]
            one_batch['concept_vads_batch'] = batch['concept_vads_batch'][j][None, :]
            one_batch['concept_vad_batch'] = batch['concept_vad_batch'][j][None, :]
            one_batch['adjacency_mask_batch'] = batch['adjacency_mask_batch'][j][None, :]
            one_batch['target_batch'] = batch['target_batch'][j][None, :]
            one_batch['target_ext_batch'] = batch['target_ext_batch'][j][None, :]
            one_batch['target_lengths'] = batch['target_lengths'][j]
            one_batch['target_emotion'] = batch['target_emotion'][j][None, :]
            one_batch['emotion_label'] = batch['emotion_label'][:, None][j]
            one_batch['emotion_widx'] = batch['emotion_widx'][:, None][j]
            one_batch['context_txt'] = [batch['context_txt'][j]]
            one_batch['target_txt'] = [batch['target_txt'][j]]
            one_batch['emotion_txt'] = [batch['emotion_txt'][j]]
            one_batch['concept_txt'] = [batch['concept_txt'][j]]
            one_batch['oovs'] = [batch['oovs'][j]]
    
            sent_g = self.decoder_greedy(one_batch, max_dec_step=30)  # sentences list, each sentence is a string.
            for i, greedy_sent in enumerate(sent_g):
                # rf = " ".join(batch["target_txt"][i])
                res[itr] = greedy_sent.split()
                gdn[itr] = one_batch["target_txt"][i]  # targets.split()
                itr += 1
        
            list_res = [r for r in res.values()]
            list_gdn = [g for g in gdn.values()]
        pred = [list_res, pred_emotion.tolist()]
        label = [list_gdn,batch["emotion_label"].cpu().numpy().tolist()]
        metric_function.evaluate(pred,label)
        # metric_function.evaluate(res, gdn)
    ...
    def predict_label(self, batch):
        ...
        gdn = {}
        itr = 0
        sent_g = self.decoder_greedy(batch, max_dec_step=30)  # sentences list, each sentence is a string.
        # for i, greedy_sent in enumerate(sent_g):
            # rf = " ".join(batch["target_txt"][i])
            # res[itr] = greedy_sent.split()
