...
class DecoderDeep(nn.Module):
    def __init__(self, dec_type, input_size, output_size, hidden_size, d_size, n_layer=1, dropout=0.5, use_cuda=False):
        ...
        if use_cuda:
            for i in range(n_layer):
                if i == 0:
                    self.w2h.append(nn.Linear(input_size, hidden_size * 4).to(device))
                    self.w2h_r.append(nn.Linear(input_size, d_size).to(device))
                else:
                    self.w2h.append(nn.Linear(input_size + i * hidden_size, hidden_size * 4).to(device))
                    self.w2h_r.append(nn.Linear(input_size + i * hidden_size, d_size).to(device))

                self.h2h.append(nn.Linear(hidden_size, hidden_size * 4).to(device))
                self.h2h_r.append(nn.Linear(hidden_size, d_size).to(device))
                self.dc.append(nn.Linear(d_size, hidden_size, bias=False).to(device))
        else:
            for i in range(n_layer):
                if i == 0:
    ...
    def _step(self, input_t, last_hidden, last_cell, last_dt, layer_idx):
        # input_t [64,1392] last_hidden[64,100] last_cell [64,100] last_dt[64,571] layer_idx=0
        '''
    ...
    def forward(self, input_var, dataset, init_hidden=None, init_feat=None, gen=False, sample_size=1):
        ...
        for t in range(max_len):
            ...
            #  output[64,64,1392]  output_prob[64,1,1392]
            self.output_prob[:, t, :] = output
            previous_out = self.logits2words(output, decoded_words, dataset, sample_size).to(device)
            vocab_t = previous_out if gen else input_var[:, t, :]  # (batch_size, output_size)

        if gen:
            decoded_words = self.truncate(decoded_words)
        return self.output_prob, decoded_words  # decode解码输出的 output_prob和gens
    ...
    def logits2words(self, output, decoded_words, dataset, sample_size):
        ...
        * take argmax if sample size == 1
        ...
        if self.USE_CUDA:
            decoded_words_t = decoded_words_t.to(device)
