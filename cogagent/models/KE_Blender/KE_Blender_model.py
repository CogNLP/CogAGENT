...
class BlenderbotSmallModel(BlenderbotSmallPreTrainedModel):
    ...
    # def evaluate(self, batch, loss_function):
...
class BlenderbotSmallForConditionalGeneration(BlenderbotSmallPreTrainedModel):
    ...
    def __init__(self, config: BlenderbotSmallConfig):
        ...
        self.encoder_tokenizer = BlenderbotSmallTokenizer.from_pretrained("/home/nlp/CogAGENT/datapath/KE_Blender_data/",
                                                            additional_special_tokens=['__defi__', '__hype__',
                                                                                       '[MASK]'])
    ...
    def loss(self, batch, loss_function):
        inputs = {'input_ids': batch["train_input_ids"], 'attention_mask': batch["train_attention_mask"],
                  'decoder_input_ids': batch["train_decoder_input_ids"], 'labels': batch["train_labels"]}
    ...
    def evaluate(self, batch, metric_function):
        dev_loss, correct, count, words, dev_text = self.predict(batch)
        metric_function.evaluate(dev_loss, correct, count, words, dev_text)

    def predict(self, batch):
        correct = 0
        count = 0
        dev_text = batch["dev_text"]
        inputs = {'input_ids': batch["dev_input_ids"], 'attention_mask': batch["dev_attention_mask"],
                  'decoder_input_ids': batch["dev_decoder_input_ids"], 'labels': batch["dev_labels"]}
        outputs = self.forward(**inputs)
        dev_loss = outputs[0]
        dev_loss = dev_loss.mean().item()
        decode_outputs = torch.argmax(outputs[1], dim=-1).view(-1)
        labels = batch["dev_labels"].view(-1)
        for i, j in zip(labels, decode_outputs):
            if i == j and i != -100:
                correct += 1
            if i != -100:
                count += 1
        words = self.word_generate(dev_text)
        return dev_loss, correct, count, words, dev_text

    def word_generate(self, batch):
#         self._move_model_to_device()
        ...
        # ]:
        # for batch in [
        #     test_data[i : i + 16] for i in range(0, len(test_data), 16)
        # ]:
